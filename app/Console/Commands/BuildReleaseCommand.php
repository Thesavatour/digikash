 ...
 *   --skip-prep      : Skip steps 1–3 (only if you already ran no-dev prep).
...
class BuildReleaseCommand extends Command
{
    protected $signature = 'release:build
        ...
        {--skip-prep : Skip prep; only use after composer install --no-dev was already run}
    ...
    /** @var list<string> */
    private array $devVendorPackagePaths = [];
    ...
    public function handle(): int
    {
        ...
        $this->devVendorPackagePaths = $this->loadDevVendorPackagePaths();
        ...
        $this->table(['Setting', 'Value'], [
            ...
            ['SQL path', $sqlInZip],
            ['Prep',     $skipPrep ? 'skipped (expects production vendor)' : 'composer no-dev · optimize:clear · unlink storage'],
    ...
    /**
     * Read Composer's lock file and return every dev-only package as a
     * vendor-relative directory path. This keeps release zips clean even if
     * someone builds with --skip-prep while local dev dependencies exist.
     *
     * @return list<string>
     */
    private function loadDevVendorPackagePaths(): array
    {
        $lockPath = $this->projectRoot.DIRECTORY_SEPARATOR.'composer.lock';
        ...
        if (! is_array($decoded) || ! isset($decoded['packages-dev']) || ! is_array($decoded['packages-dev'])) {
            return [];
        }
        ...
        foreach ($decoded['packages-dev'] as $package) {
            ...
            $name = strtolower(str_replace('\\', '/', trim($package['name'], '/')));
    ...
    private function isExcluded(string $relative): bool
    {
        ...
        if ($this->isDevVendorPackagePath($relative) || $this->isDisposableStoragePath($relative)) {
            return true;
        }
    ...
    private function isDevVendorPackagePath(string $relative): bool
    {
        if ($this->devVendorPackagePaths === []) {
            return false;
        }

        $relative = strtolower(trim(str_replace('\\', '/', $relative), '/'));

        foreach ($this->devVendorPackagePaths as $packagePath) {
            if ($relative === $packagePath || str_starts_with($relative.'/', $packagePath.'/')) {
                return true;
            }
        }
    ...
    private function isDisposableStoragePath(string $relative): bool
    {
        $relative = strtolower(trim(str_replace('\\', '/', $relative), '/'));
        ...
        foreach ($alwaysExcluded as $prefix) {
            if ($relative === $prefix || str_starts_with($relative.'/', $prefix.'/')) {
                return true;
            }
        }

        $temporarySegments = ['temp', 'temporary', 'tmp'];
        $segments          = explode('/', $relative);

        return array_intersect($segments, $temporarySegments) !== [];
    }
