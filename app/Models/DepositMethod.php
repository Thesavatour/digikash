...
class DepositMethod extends Model
{
    ...
    protected $casts = [
        ...
        'agent_charge_type'    => FixPctType::class,
     ...
     */
    public function getFieldsAttribute(mixed $value): array
    {
        return $this->normalizeFields($value);
    }

    public function setFieldsAttribute(mixed $value): void
    {
        $this->attributes['fields'] = json_encode($this->normalizeFields($value));
    }
    ...
    {
        return $this->belongsToMany(Merchant::class, 'merchant_deposit_methods')
            ->withTimestamps();
    }
    ...
    public function getConversionRateAttribute(): string
    {
        ...
        if ($this->attributes['conversion_rate_live'] && $this->attributes['type'] === 'auto') {
            ...
            // Format using site-wide decimal precision.
            $decimals = (int) (setting('site_decimal', 2));

            return number_format($currencyConversionService->convertCurrency(1, siteCurrency(), $this->attributes['currency']), $decimals, '.', '');
        }
    ...
    public function getChargeAttribute(): float
    {
        ...
        if ($user->isMerchant() && ($this->attributes['merchant_charge'] ?? null) !== null) {
            return (float) $this->attributes['merchant_charge'];
        }

        if ($user->isAgent() && ($this->attributes['agent_charge'] ?? null) !== null) {
            return (float) $this->attributes['agent_charge'];
        }

        if (($this->attributes['user_charge'] ?? null) !== null) {
    ...
    public function getChargeTypeAttribute(): string
    {
        ...
        if ($user->isMerchant() && ($this->attributes['merchant_charge_type'] ?? null) !== null) {
            return $this->attributes['merchant_charge_type'];
        }

        if ($user->isAgent() && ($this->attributes['agent_charge'] ?? null) !== null) {
            return $this->attributes['agent_charge_type'];
        }

        if (($this->attributes['user_charge_type'] ?? null) !== null) {
