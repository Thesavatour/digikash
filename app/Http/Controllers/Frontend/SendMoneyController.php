...
class SendMoneyController extends Controller
{
    ...
    {
        $prefilledRecipient = $request->string('recipient')->trim()->toString();

        $featureManager = app(FeatureManager::class);
        $accessRule     = $featureManager->find('send_money')
            ?->accessRules
            ->firstWhere('panel', $featureManager->currentPanel());

        $recipientPlaceholder = $this->buildRecipientPlaceholder($accessRule);

        return view('frontend.user.send_money.create', compact('prefilledRecipient', 'recipientPlaceholder'));
    }
     ...
     */
    public function store(SendMoneyRequest $request, SendMoneyHandler $sendMoneyHandler): RedirectResponse
    {
        ...
        try {
            // Fetch sender wallet
            $senderWallet = $request->user()
                ->wallets()
                ->with(['currency.roles', 'user'])
                ->findOrFail($walletId);

            if (! $senderWallet->status || ! $senderWallet->hasCurrencyRole(CurrencyRole::SENDER)) {
                notifyEvs('error', __('Selected wallet cannot send money.'));
            ...
            $lookup = Wallet::resolveRecipientWallet($recipient, $senderWallet->currency_id, $accessRule);
            ...
            if (! $recipientWallet->status) {
                notifyEvs('error', __('Recipient wallet is not active.'));
