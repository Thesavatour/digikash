...
use App\Http\Requests\PaymentGateway\FilterPaymentGatewayRequest;
...
class PaymentGatewayController extends BaseController
{
    ...
    public function index(FilterPaymentGatewayRequest $request)
    {
        $filters             = $request->validated();
        $gateways            = PaymentGateway::query()->orderBy('id')->get();
        $paymentGateways     = $this->filteredGateways($gateways, $filters);
        $gatewayStats        = $this->gatewayStats($gateways);
        $availableCurrencies = $this->availableCurrencies($gateways);
        $filtersActive       = collect($filters)->filter(fn (mixed $value): bool => filled($value))->isNotEmpty();

        return view('backend.payment_gateway.index', compact('paymentGateways', 'gatewayStats', 'availableCurrencies', 'filters', 'filtersActive'));
    }

    /**
     * @param  Collection<int, PaymentGateway>|null               $gateways
     * @return array{active: int, withdraw: int, currencies: int}
     */
    private function gatewayStats($gateways = null): array
    {
        $gateways ??= PaymentGateway::query()->select(['id', 'code', 'status', 'withdraw_field', 'currencies'])->get();
     ...
     */
    private function filteredGateways($gateways, array $filters): LengthAwarePaginator
    {
        $filtered = $gateways
            ->when($filters['search'] ?? null, function ($collection, string $search) {
                $needle = mb_strtolower($search);

                return $collection->filter(function (PaymentGateway $gateway) use ($needle): bool {
                    $haystack = collect([
                        $gateway->name,
                        $gateway->code,
                        ...($gateway->currencies ?? []),
                    ...
                    return $haystack->contains(fn (string $value): bool => str_contains($value, $needle));
                });
            })
            ->when($filters['status'] ?? null, function ($collection, string $status) {
                return $collection->filter(fn (PaymentGateway $gateway): bool => $status === 'active'
                    ? $gateway->status
                    : ! $gateway->status);
            })
            ->when($filters['capability'] ?? null, function ($collection, string $capability) {
                return $collection->filter(fn (PaymentGateway $gateway): bool => $capability === 'withdraw'
                    ? $gateway->withdraw_available
                    : ! $gateway->withdraw_available);
            })
            ->when($filters['currency'] ?? null, function ($collection, string $currency) {
                return $collection->filter(fn (PaymentGateway $gateway): bool => collect($gateway->currencies ?? [])
     ...
     */
    private function availableCurrencies($gateways)
    {
        return $gateways
            ->flatMap(fn (PaymentGateway $gateway): array => $gateway->currencies ?? [])
            ->map(fn (mixed $currency): string => strtoupper((string) $currency))
    ...
    private function testGatewayCredentials(string $code, array $credentials, string $testMode): array
    {
        return match ($code) {
            ...
            'mercadopago' => $this->testMercadoPagoCredentials($credentials, $testMode),
    ...
    private function testMercadoPagoCredentials(array $credentials, string $testMode): array
    {
        ...
        return $this->gatewayTestResponse($response, 'Mercado Pago', $this->testModeLabel($credentials, $testMode));
    }
