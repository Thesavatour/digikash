...
class SubscriptionPlanController extends BaseController
{
    ...
    public function destroy(SubscriptionPlan $plan): RedirectResponse
    {
        ...
        // `user_subscriptions.subscription_plan_id` and
        // `subscription_transactions.subscription_plan_id` are
        // restrictOnDelete, so a bare $plan->delete() throws a foreign-key
        // violation whenever the plan ever had a (now expired/cancelled)
        // subscriber — and any rows left dangling would null out
        // `$subscription->plan` and 500 the user's subscription history.
        //
        // Clear the historical subscriptions + their transactions first
        // (plan prices + features cascade automatically), all atomically.
        DB::transaction(function () use ($plan): void {
            $subscriptionIds = $plan->subscriptions()->pluck('id');
            ...
            // Defensive: drop any transaction that references the plan
            // directly without a surviving subscription row.
            SubscriptionTransaction::query()
    ...
    private function formData(SubscriptionPlan $plan): array
    {
        ...
        return [
            ...
            'featureTypes'       => [
                'limit'   => __('Limit (amount cap)'),
                'quota'   => __('Quota (count allowance)'),
                'access'  => __('Access (enabled/disabled)'),
                'benefit' => __('Benefit (plan perk)'),
            ],
            'resetCycles' => [
                ''        => __('No reset'),
                'daily'   => __('Daily'),
    ...
    private function syncFeatures(SubscriptionPlan $plan, array $features): void
    {
        ...
        foreach ($features as $index => $featureData) {
            ...
            $featureType  = $featureData['feature_type']  ?? 'limit';
            $featureValue = $featureData['feature_value'] ?? ($featureType === 'access' ? 'enabled' : '1');
            ...
            $plan->features()->updateOrCreate(
                ...
                [
                    ...
                    'reset_cycle'   => ($featureData['reset_cycle'] ?? null) ?: null,
