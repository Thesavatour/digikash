...
class PluginController extends BaseController
{
    ...
    private function testPluginCredentials(Plugin $plugin, array $credentials): array
    {
        return match ($plugin->code) {
            ...
            'google-login'     => $this->testGoogleLoginCredentials($credentials),
            'apple-login'      => $this->testAppleLoginCredentials($credentials),
    ...
    private function testGoogleLoginCredentials(array $credentials): array
    {
        if (! filled($credentials['client_id'] ?? null) || ! filled($credentials['client_secret'] ?? null)) {
            return [
                'status'  => 'error',
                'message' => __('Client ID and Client Secret are required for Google Sign-In.'),
            ];
        }

        return [
            'status'  => 'success',
            'message' => __('Google Sign-In credentials look complete. Turn the plugin on, then test Continue with Google on the user login page.'),
        ];
    }

    private function testAppleLoginCredentials(array $credentials): array
    {
        ...
        if ($missing !== []) {
            return [
                'status'  => 'error',
                'message' => __('Missing Apple Sign-In fields: :fields', [
        ...
        return [
            'status'  => 'success',
            'message' => __('Apple Sign-In credentials look complete. Turn the plugin on, then test Continue with Apple on the user login page.'),
        ];
    }
