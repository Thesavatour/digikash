...
use App\Support\PaymentMethodCodeGenerator;
...
class DepositMethodController extends BaseController
{
    ...
    public function index()
    {
        ...
        $paymentMethods = DepositMethod::where('type', $type)->paginate(10)->withQueryString();
    ...
    private function prepareData(StoreUpdateRequest $request, ?DepositMethod $paymentMethod = null)
    {
        ...
        $paymentGateway = $request->filled('payment_gateway_id')
            ? PaymentGateway::query()->findOrFail((int) $request->input('payment_gateway_id'))
            : null;

        $methodCode = $paymentGateway
            ? PaymentMethodCodeGenerator::forGatewayCurrency($paymentGateway, (string) $request->input('currency'), DepositMethod::class, $paymentMethod?->id)
            : $validated['method_code'];

        $logo = $request->hasFile('logo')
            ? self::uploadImage($request->file('logo'), $paymentMethod?->logo)
            : ($paymentMethod?->logo ?? $paymentGateway?->logo ?? 'general/static/default/payment-gateway.png');
        ...
        return array_merge($validated, [
            ...
            'receive_payment_details' => $request->filled('receive_payment_details') ? Purifier::clean(htmlspecialchars_decode($request->input('receive_payment_details'))) : '',
            // Get the status as a boolean
            'status' => $request->boolean('status'),
            // Agent charge is optional — store null when blank so agents fall back to the regular user charge
            'agent_charge'      => $request->filled('agent_charge') ? $request->input('agent_charge') : null,
            'agent_charge_type' => $request->input('agent_charge_type', 'percent'),
