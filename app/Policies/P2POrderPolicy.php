...
class P2POrderPolicy
{
    ...
    /**
     * Chat stays open while the trade can still move (pending, paid, or
     * disputed); the transcript remains viewable through view() afterwards.
     */
    public function chat(User $user, Order $order): bool
    {
        return $this->view($user, $order)
            && in_array($order->status->value, ['PENDING', 'PAID', 'DISPUTED'], true);
    }
