	...
	<style>
        ...
        * {
        ...
        .installer-aside {
            ...
            padding: 24px;
            color: #ffffff;
            background: linear-gradient(180deg, var(--side) 0%, var(--side-deep) 100%);
            display: flex;
        ...
        .install-error {
            ...
            border-radius: 12px;
            background: radial-gradient(circle at 100% 0%, rgba(189, 45, 45, .10), transparent 55%),
        ...
        .rv__plan {
            ...
            border-radius: 12px;
            background: radial-gradient(circle at 0% 0%, rgba(var(--primary-rgb), 0.06), transparent 50%),
        ...
        .rv__card--app {
            --rv-card-accent: #2563eb;
        }

        .rv__card--db {
            --rv-card-accent: #7c3aed;
        }

        .rv__card--admin {
            --rv-card-accent: #059669;
        }

        .rv__card--data {
            --rv-card-accent: #d97706;
        }
        ...
        .rv__ready {
            ...
            border-radius: 14px;
            background: radial-gradient(circle at 100% 0%, rgba(5, 150, 105, 0.1), transparent 55%),
		...
		<div class="brand">
                <span class="brand-mark" aria-hidden="true">
                    <svg viewBox="0 0 32 32" role="img">
                        ...
                        <path d="M11 14h11a3 3 0 0 1 3 3v4a3 3 0 0 1-3 3H11a3 3 0 0 1-3-3v-8a2 2 0 0 1 2-2h9" fill="none" stroke="#fff" stroke-width="2.3" stroke-linecap="round"
						...
						<div class="fields">
							...
							<div class="field">
								...
								<input id="app_url"
								       ...
								       maxlength="255"
								       class="is-locked"
								       data-review-source="app_url"
								       data-admin-url-base
								       data-locked-url>
								<span class="field-help">{{ __('Detected automatically from your current address. No need to change anything — it is already set for you.') }}</span>
							...
							<div class="field">
								...
								<span
									class="field-help">{{ __('Used as the site-wide wallet currency, displayed across dashboards. Add or change currencies later from Admin → Currencies. Default: :default.', ['default' => $defaults['default_currency_code']]) }}</span>
					...
					<section class="step-panel" data-step-panel="2">
						...
						<div class="fields">
							<div class="field is-wide">
								...
								<input id="envato_purchase_code" name="envato_purchase_code" value="{{ old('envato_purchase_code') }}" @if($envatoRequired ?? true) required @endif maxlength="80"
								       placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" data-envato-purchase-code>
								@error('envato_purchase_code') <span class="error">{{ $message }}</span> @enderror
								@unless($envatoRequired ?? true)
									<span class="help">{{ __('License verification is disabled for this install. You can continue without a purchase code.') }}</span>
								@endunless
							</div>
						</div>
					...
					<section class="step-panel" data-step-panel="3">
						...
						<div class="fields">
							...
							<div class="field">
								...
								<input id="db_database" name="db_database" value="{{ old('db_database', $defaults['db_database']) }}" required maxlength="255" data-db-database data-db-watch
								       data-review-source="db_database">
...
<script>
    ...
    (function () {
        ...
        const envatoRequired = @json((bool) ($envatoRequired ?? true));
        let licenseReady = !envatoRequired;
        let licenseMessage = envatoRequired ? @json(__('Not verified')) : @json(__('License verification disabled'));
        ...
        function setLicenseUntested() {
            if (!envatoRequired) {
                licenseReady = true;
                licenseMessage = @json(__('License verification disabled'));
                licenseStatusText.textContent = @json(__('License verification is disabled. You can continue.'));
                licenseActions.className = 'db-actions is-success';
        ...
        // Keep the auto-detected Application URL immutable. This is only a
        // front-end deterrent for casual DevTools edits — the server always
        // overwrites app_url with the real request host on submit, so a
        // tampered value can never reach the database or the license server.
        const lockedUrlField = document.querySelector('[data-locked-url]');

        if (lockedUrlField) {
            const lockedUrlValue = lockedUrlField.value;

            const enforceUrlLock = () => {
                if (lockedUrlField.value !== lockedUrlValue) {
                    lockedUrlField.value = lockedUrlValue;
                    updateReview();
                }

                if (!lockedUrlField.hasAttribute('readonly')) {
                    lockedUrlField.setAttribute('readonly', 'readonly');
                }
            };

            // `readonly` already blocks typing/paste; these guard against
            // programmatic edits (DevTools) while still allowing copy.
            ['input', 'change'].forEach((evt) => {
                lockedUrlField.addEventListener(evt, enforceUrlLock);
            });

            window.setInterval(enforceUrlLock, 500);
        }
    ...
    (function () {
        ...
        if (pwConfirm) {
            pwConfirm.addEventListener('input', refresh);
        }
        refresh();
    })();
</script>
