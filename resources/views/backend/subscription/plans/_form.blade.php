	...
	<div class="row g-4 align-items-start">
		...
		<div class="col-12">
			...
			<div class="sa-card plan-features-card">
                ...
                <div class="sa-card__body">
                    <div class="plan-features-scope-banner mb-3">
                        <div class="plan-features-scope-banner__icon">
                            <i class="fa-solid fa-shield-alt" aria-hidden="true"></i>
                        </div>
                        <div class="plan-features-scope-banner__body">
                            <div class="plan-features-scope-banner__title">@lang('Plans configure access levels - features are global')</div>
                            <p class="plan-features-scope-banner__copy mb-0">
                                @lang('Feature Management keeps modules visible or hidden globally. Subscription plans decide what a subscriber can do inside enabled modules: access, quotas, limits, and benefits.')
                                <a href="{{ route('admin.features.index') }}" class="plan-features-scope-banner__link">
                                    @lang('Manage global features') <i class="fas fa-arrow-right"></i>
                                </a>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="plan-features-card__intro mb-4">
                        <div class="plan-features-card__intro-copy">
                            <div class="plan-features-card__eyebrow">@lang('Plan Entitlements Builder')</div>
                            <p class="mb-0">@lang('Add the access rules, numeric caps, quotas, and premium benefits that differentiate this plan. Drag rows to control the order on plan cards.')</p>
                        </div>
                    </div>
                    ...
                    <div class="feat-type-guide mb-4">
                        <div class="feat-type-guide__title">
                            ...
                            <div>
                                @lang('Row Types')
                                <div class="feat-type-guide__subtitle">@lang('Pick the business meaning first, then set a clean value for admins and users.')</div>
                            </div>
                        </div>
                        <div class="feat-type-guide__list">
                            <div class="feat-type-guide__item">
                                ...
                                <span class="feat-type-guide__desc">@lang('Amount caps such as monthly withdrawal, monthly send, or wallet balance.')</span>
                            </div>
                            <div class="feat-type-guide__item">
                                <span class="feat-type-guide__badge feat-type-guide__badge--quota">@lang('Quota')</span>
                                <span class="feat-type-guide__desc">@lang('Count-based allowances such as P2P ads, P2P trades, or virtual card slots.')</span>
                            </div>
                            <div class="feat-type-guide__item">
                                <span class="feat-type-guide__badge feat-type-guide__badge--access">@lang('Access')</span>
                                <span class="feat-type-guide__desc">@lang('Enabled or disabled premium actions, such as P2P trading, Wallet Earn, virtual cards, or API access.')</span>
                            </div>
                            <div class="feat-type-guide__item">
                                <span class="feat-type-guide__badge feat-type-guide__badge--benefit">@lang('Benefit')</span>
                                <span class="feat-type-guide__desc">@lang('Text perks such as priority support, faster withdrawals, or dedicated account help.')</span>
                            </div>
                        </div>
                    </div>
			...
			<div class="sa-card plan-settings-shell">
				...
				<div class="sa-card__body">
					<div class="plan-settings-grid">
						<div class="plan-policy-fields">
							<div>
								<label class="form-label" for="trial-days">@lang('Free Trial Days')</label>
								<div class="input-group">
								...
								<div class="form-text">@lang('Paid plans start as a free trial when this is greater than 0. Set 0 to charge immediately.')</div>
							</div>
							<div>
								<label class="form-label" for="grace-days">@lang('Grace Period Days')</label>
								<div class="input-group">
									<input type="number" name="grace_days" id="grace-days"
									       min="0" max="90"
								...
								<div class="form-text">@lang('Keeps access active briefly if renewal or trial conversion payment fails.')</div>
							</div>
						</div>
...
@push('styles')
	<style>
        ...
        .plan-features-scope-banner__icon {
            flex-shrink: 0;
            width: 36px;
            height: 36px;
            border-radius: 10px;
        ...
        .plan-features-scope-banner__title {
            margin-bottom: 4px;
            font-size: 13px;
            font-weight: 700;
            color: #0f172a;
        ...
        .plan-features-scope-banner__copy {
            font-size: 12.5px;
            line-height: 1.55;
            color: #334155;
        }

        .plan-features-scope-banner__link {
            display: inline-flex;
            align-items: center;
            gap: 4px;
            margin-left: 6px;
        ...
        .plan-features-scope-banner__link:hover {
            color: #1e40af;
            text-decoration: underline;
        }

        .plan-features-scope-banner__link i {
            font-size: 10px;
        }
    ...
    .feat-type-guide__item {
        ...
        .feat-type-guide__badge--access {
            background: #dbeafe;
            color: #1d4ed8;
        }

        .feat-type-guide__badge--benefit {
            background: #f3e8ff;
            color: #7e22ce;
        }
    ...
    .feature-row__card {
        display: grid;
        grid-template-columns: 28px minmax(180px, 2fr) minmax(180px, 1.5fr) minmax(120px, .9fr) minmax(180px, 1.2fr) auto;
        gap: 8px;
        align-items: start;
        padding: 9px 10px;
        border: 1px solid #dbe7ff;
	...
	<script>
        ...
        (function () {
            ...
            function applyFeatureTypeState(row) {
                ...
                switchValueField(row, typeSelect.value || 'limit');
            }
            ...
            container.addEventListener('change', function (e) {
                ...
                const accessSelect = e.target.closest('.feature-access-select');
                if (accessSelect) {
                    updateAccessValue(accessSelect.closest('.feature-row'), accessSelect.value);
                }
            });

            function switchValueField(row, type) {
                const valText = row.querySelector('.feature-val-text');
                const accessSelect = row.querySelector('.feature-access-select');
                if (!valText) {
                    return;
                }
                ...
                if (type === 'access') {
                    ...
                    accessSelect?.classList.remove('d-none');
                    updateAccessValue(row, accessSelect?.value || valText.value || 'enabled');
                    return;
                }

                accessSelect?.classList.add('d-none');
                ...
                valText.placeholder = {
                    quota: '{{ __('e.g. 50 / unlimited') }}',
                    benefit: '{{ __('e.g. Priority support / Faster withdrawals') }}',
                    limit: '{{ __('e.g. $10,000 / unlimited') }}',
                }[type] || '{{ __('e.g. $10,000 / unlimited') }}';
            }

            function updateAccessValue(row, value) {
                const valText = row.querySelector('.feature-val-text');
                const accessSelect = row.querySelector('.feature-access-select');
                ...
                valText.value = value === 'disabled' ? 'disabled' : 'enabled';

                if (accessSelect) {
                    accessSelect.value = valText.value;
                }
            }
