                                ...
                                @forelse($templates as $t)
                                    @php
                                        ...
                                        $editPayload = $t->only([
                                            ...
                                            'default_amount', 'status', 'sort_order', 'image',
                                        ]);
                                        $editPayload['image_url'] = $t->imageUrl();
                                    @endphp
                                    <tr data-id="{{ $t->id }}">
                                        @can('gift-card-template-manage')
    ...
    @push('scripts')
        <script>
            ...
            (function () {
                ...
                const SITE_BRAND = @json((string) setting('site_title', 'DigiKash'));
                ...
                const imageInput  = modalEl.querySelector('input.imageUpload[name="image"]');
                const imagePreviewBox = modalEl.querySelector('#image');
                const PLACEHOLDER_IMAGE = @json(asset('general/static/default/placeholder.png'));
                let objectUrl = null;
                ...
                function syncImageUploaderPreview() {
                    if (! imagePreviewBox) return;
                    const url = state.image_url || PLACEHOLDER_IMAGE;
                ...
                function applyState() {
                    ...
                    // Live preview + uploader thumbnail
                    syncImageUploaderPreview();
                ...
                function renderPreview() {
                    ...
                    const motifSvg = imageUrl ? '' : renderMotif(def.motif, uid);
                    const photoLayer = imageUrl
                        ? `<div style="position:absolute;inset:0;background-image:url('${String(imageUrl).replace(/'/g, "\\'")}');background-size:cover;background-position:center;background-repeat:no-repeat;"></div>
                           <div style="position:absolute;inset:0;background:linear-gradient(180deg, rgba(15,23,42,.28) 0%, rgba(15,23,42,.55) 55%, rgba(15,23,42,.72) 100%);pointer-events:none;"></div>`
                        : '';
                    const cardBg = imageUrl ? '#0F172A' : bg;

                    return `
                        <div style="width:${w}px; height:${h}px; position:relative; background:${cardBg}; color:${ink}; border-radius:${w >= 320 ? 18 : 12}px; overflow:hidden; box-shadow:0 26px 50px -12px rgba(15,23,42,.45), 0 12px 24px -8px rgba(15,23,42,.25), inset 0 0 0 1px rgba(255,255,255,.10); font-family:'Plus Jakarta Sans', system-ui, sans-serif;">
                            ${photoLayer}
                            ${motifSvg}
                            ...
                            <div style="position:absolute; left:${(w*0.05).toFixed(1)}px; right:${(w*0.05).toFixed(1)}px; bottom:${(w*0.05).toFixed(1)}px; display:flex; justify-content:space-between; align-items:flex-end; gap:${(w*0.04).toFixed(1)}px;">
                                ...
                                <div style="min-width:0; flex:1; text-align:right;">
                                    ...
                                    <div style="font-size:${(w*0.045).toFixed(1)}px; font-weight:700; margin-top:${(w*0.005).toFixed(1)}px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(SITE_BRAND)}</div>
                                </div>
                            </div>
                        </div>
                    `;
                ...
                wrapper.querySelectorAll('.js-gctm-edit').forEach((btn) => {
                    btn.addEventListener('click', () => {
                        try {
                            ...
                            state.image_url        = payload.image_url || null;
                ...
                if (imageInput) {
                    imageInput.addEventListener('change', () => {
                        ...
                        if (! file) {
                            // Keep previously saved image when the file picker is cleared.
                            state.image_url = state.image
