@php
    ...
    $cwd = base_path();
    $storageLinkAdmin = auth('admin')->user();
    $canRunStorageLink = $storageLinkAdmin?->canAny(['site-setting-view', 'site-setting-update']) ?? false;
@endphp
    ...
    <div class="alert alert-danger d-flex align-items-center gap-3 p-4 mb-4 shadow-sm" role="alert" style="border-left: 5px solid #e55353;">
        ...
        <div>
            ...
            <div class="d-flex flex-wrap align-items-center gap-2">
                @if($canRunStorageLink)
                    <form method="POST" action="{{ route('admin.settings.site.storage-link') }}" data-storage-link-form>
                        @csrf
                        <button type="submit"
                            ...
                            <span data-storage-link-button-label>{{ __('Run Storage Link') }}</span>
                        </button>
                    </form>
                @endif
