...
@section('auth-content')
	@php
		// Pre-filled demo credentials are a LOCAL developer convenience only.
		// Both values default to empty in config, so the committed source ships
		// NO credentials — nothing reaches a client via git tag, update package,
		// or release zip. They only fill when the developer sets APP_DEMO=true
		// plus DEMO_ADMIN_EMAIL / DEMO_ADMIN_PASSWORD in their own local .env.
		$isDemo         = (bool) config('app.demo', false);
		$demoAdminEmail = $isDemo ? (string) config('app.demo_admin_email') : '';
		$demoAdminPass  = $isDemo ? (string) config('app.demo_admin_password') : '';
	@endphp
		...
		<div class="admin-auth-field mb-3">
			...
			<div class="input-group admin-auth-input-group">
				...
				<input class="form-control admin-auth-input" id="email" type="email"
				       name="email"
				       value="{{ old('email', $demoAdminEmail) }}"
		...
		<div class="admin-auth-field mb-3">
			...
			<div class="input-group admin-auth-input-group">
				...
				<input class="form-control admin-auth-input"
				       ...
				       @if($demoAdminPass !== '') value="{{ $demoAdminPass }}" @endif
