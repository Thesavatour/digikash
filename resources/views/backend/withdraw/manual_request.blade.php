...
@section('withdraw_header')
    <x-admin-feature-header
        :title="__('Manual Withdraw Requests')"
        :subtitle="__('Review, approve, or reject all manual withdraw requests submitted by users.')"
        icon="request"
    />
@endsection
                    ...
                    @forelse($withdrawRequests as $transaction)
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <img class="rounded-circle shadow-sm me-2" width="36" height="36"
                                         src="{{ asset($transaction->user->avatar_alt) }}" alt="User Avatar" loading="lazy">
                                    <div>
                        ...
                        @empty
                            <tr>
                                <td colspan="5">
                                    <x-admin-not-found
                                        :title="__('No withdraw requests found')"
                                        :message="__('Manual withdraw requests will appear here when users submit them.')"
                                        icon="fa-wallet"
                                    />
                                </td>
                            </tr>
