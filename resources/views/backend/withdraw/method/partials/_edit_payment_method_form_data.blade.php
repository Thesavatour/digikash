...
<form action="{{ route('admin.withdraw.method.update', $paymentMethod->id) }}" method="post"
    ...
    @if($paymentMethod->type == App\Enums\MethodType::MANUAL)
        <div class="row mb-3">
            <div class="col-lg-6 col-md-6 col-12">
                @include('backend.partials._method_code_uniqueness_hint', ['value' => $paymentMethod->method_code, 'placeholder' => 'Ex: natcash-htg-payout', 'context' => 'withdraw'])
            </div>
    ...
    @if($paymentMethod->type == App\Enums\MethodType::MANUAL)
        ...
        <div class="dynamic-fields mb-3">
            ...
            <div class="append-new-field-edit dynamic-fields__list" data-empty="{{ __('No fields yet. Click Add Field to collect payout details from the customer.') }}">@foreach($paymentMethod->fields as $key => $value)@include('backend.withdraw.method.partials._method_append_form_field', ['key' => $key, 'field' => $value])@endforeach</div>
        </div>
    @endif
