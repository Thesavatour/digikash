@php
    $modalId = 'withdraw-review-'.$transaction->id;
...
<div class="modal fade manual-request-review-modal manual-request-review-modal--withdraw" id="review-{{ $transaction->id }}" tabindex="-1" aria-labelledby="{{ $modalLabelId }}" aria-hidden="true">
            ...
            <div class="modal-header manual-request-review-modal__header">
                <div class="manual-request-review-modal__identity">
                    ...
                    <div class="manual-request-review-modal__title-group">
                        <span class="manual-request-review-modal__eyebrow">{{ __('Manual Withdrawal Review') }}</span>
            ...
            <form action="{{ route('admin.withdraw.request-action') }}" method="post">
                ...
                <div class="modal-body manual-request-review-modal__body">
                    <div class="manual-request-review-modal__callout" role="status">
                        ...
                        <div>
                            <strong>{{ __('Verify the destination account before approving.') }}</strong>
                            <span>{{ __('Approving completes the withdrawal; rejecting cancels the manual request.') }}</span>
                        </div>
                    </div>
                    ...
                    <section class="manual-request-review-modal__section" aria-labelledby="{{ $modalId }}-summary-title">
                        <div class="manual-request-review-modal__section-head">
                            ...
                            <div>
                                <h6 class="manual-request-review-modal__section-title" id="{{ $modalId }}-summary-title">{{ __('Withdrawal Summary') }}</h6>
                    ...
                    <section class="manual-request-review-modal__section" aria-labelledby="{{ $modalId }}-details-title">
                        <div class="manual-request-review-modal__section-head">
                            <span class="manual-request-review-modal__section-icon" aria-hidden="true">
                                <i class="fa-solid fa-building-columns"></i>
                            </span>
                            <div>
                                <h6 class="manual-request-review-modal__section-title" id="{{ $modalId }}-details-title">{{ __('Submitted Account Details') }}</h6>
                                <p class="manual-request-review-modal__section-subtitle">{{ __('User-provided payout account references, notes, and files.') }}</p>
                            </div>
                        </div>
                                        ...
                                        <div class="manual-request-review-modal__proof-file">
                                            <div class="manual-request-review-modal__proof-file-head">
                                                ...
                                                <div>
                                                    ...
                                                    <span>{{ __('Uploaded account document') }}</span>
                                                </div>
                                            </div>
                                            ...
                                            @if(!empty($fieldValue))
                                                <a href="{{ asset($fieldValue) }}" target="_blank" rel="noopener" class="manual-request-review-modal__proof-preview">
                                                    ...
                                                    <span>{{ __('Open full document') }}</span>
                                                </a>
                                            @else
                        ...
                        @else
                            <x-admin-not-found
                                :title="__('No submitted account details')"
                                :message="__('The user did not attach any manual withdrawal details to this request.')"
                                icon="fa-wallet"
