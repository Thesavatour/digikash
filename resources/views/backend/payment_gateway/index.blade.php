...
@section('content')
	<div class="pgw-page my-3">
		...
		<div class="pgw-board">
			...
			<form method="GET" action="{{ route('admin.payment.gateway.index') }}" class="pgw-filter" role="search">
				<div class="pgw-filter__search">
					<label class="form-label" for="pgw-search">{{ __('Search gateways') }}</label>
					<div class="pgw-filter__input">
						...
						<input type="search"
						       ...
						       placeholder="{{ __('Name, code, or currency') }}"
						       autocomplete="off">
					</div>
				</div>

				<div class="pgw-filter__field">
					<label class="form-label" for="pgw-status">{{ __('Status') }}</label>
					<select id="pgw-status" name="status" class="form-select">
				...
				<div class="pgw-filter__field">
					<label class="form-label" for="pgw-capability">{{ __('Capability') }}</label>
					<select id="pgw-capability" name="capability" class="form-select">
						<option value="">{{ __('All capabilities') }}</option>
						<option value="withdraw" @selected(($filters['capability'] ?? null) === 'withdraw')>{{ __('Withdraw available') }}</option>
						<option value="deposit_only" @selected(($filters['capability'] ?? null) === 'deposit_only')>{{ __('Deposit only') }}</option>
					</select>
				</div>

				<div class="pgw-filter__field">
					<label class="form-label" for="pgw-currency">{{ __('Currency') }}</label>
					<select id="pgw-currency" name="currency" class="form-select">
				...
				<div class="pgw-filter__actions">
					<button type="submit" class="btn btn-primary">
						<i class="fa-solid fa-filter" aria-hidden="true"></i>
						{{ __('Filter') }}
					</button>
					@if($filtersActive)
						<a href="{{ route('admin.payment.gateway.index') }}" class="btn btn-light">
			...
			<div class="pgw-grid">
				...
				@empty
					<x-admin-not-found
						:title="$filtersActive ? __('No gateways match these filters') : __('No payment gateways found')"
						:message="$filtersActive ? __('Try a broader search, clear one filter, or reset the directory view.') : __('Gateway records will appear here after they are configured.')"
						icon="fa-credit-card"
						:action-url="$filtersActive ? route('admin.payment.gateway.index') : null"
						:action-label="$filtersActive ? __('Reset Filters') : null"
