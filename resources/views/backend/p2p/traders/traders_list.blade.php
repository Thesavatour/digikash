...
@section('p2p_content')
    ...
    <div class="p2p-refresh">
        <section class="fb-card pa-table-card">
            ...
            @if($hasTraders)
                <div class="fb-toolbar fb-toolbar--directory">
                    <div class="fb-search fb-search--compact">
                        <i class="fa-solid fa-magnifying-glass" aria-hidden="true"></i>
                        <input type="search" id="fb-traders-search" placeholder="{{ __('Filter loaded traders...') }}" aria-label="{{ __('Filter loaded traders') }}">
                    </div>
                ...
                <div class="fb-card__body fb-card__body--flush">
                    <div class="fb-table table-responsive">
                        ...
                        <tbody>
                                ...
                                <tr class="{{ $isSuspended ? 'is-muted' : '' }}"
                                    ...
                                    <td class="text-end text-nowrap">
                                        @if(($s['avg_rating'] ?? null) !== null)
                                            ...
                                            <span class="fb-muted-xs">({{ number_format((int) ($s['feedback_count'] ?? 0)) }})</span>
                                        @else
                                            <span class="fb-empty-mark">-</span>
                                        @endif
                                    </td>
                                    <td class="text-end">
                            ...
                            @endforeach
                            <tr data-trader-empty class="d-none">
                                <td colspan="8">
                                    <x-admin-not-found
                                        class="admin-not-found--compact"
                                        :title="__('No loaded traders match this filter')"
                                        :message="__('Clear the local filter or use the page search to query all traders.')"
                                        icon="fa-users"
                                    />
                                    <div class="text-center pb-3">
                                        <button type="button" class="fb-btn fb-btn--ghost fb-btn--sm" data-trader-filter-reset>
                                            <i class="fa-solid fa-arrow-rotate-left" aria-hidden="true"></i>
                                            <span>@lang('Clear filter')</span>
                                        </button>
                                    </div>
                                </td>
                            </tr>
...
@push('scripts')
    <script>
        ...
        (function ($) {
            const $rows = $('[data-trader-row]');

            if (!$rows.length) {
                return;
            }

            const $search = $('#fb-traders-search');
            const $filters = $('[data-fb-traders-filter]');
            const $empty = $('[data-trader-empty]');
            ...
            function applyTraderFilter() {
                const query = normalize($search.val());
                let visibleCount = 0;

                $rows.each(function () {
                    const $row = $(this);
                    const status = normalize($row.data('trader-status'));
                    const haystack = normalize($row.data('trader-search'));
            ...
            $search.on('input', applyTraderFilter);

            $filters.on('click', function () {
                activeFilter = String($(this).data('fb-traders-filter') || 'all');
            ...
            $('[data-trader-filter-reset]').on('click', function () {
                $search.val('');
                activeFilter = 'all';
                ...
                $filters
                    .removeClass('is-active')
                    .attr('aria-pressed', 'false')
                    .filter('[data-fb-traders-filter="all"]')
                    .addClass('is-active')
                    .attr('aria-pressed', 'true');
