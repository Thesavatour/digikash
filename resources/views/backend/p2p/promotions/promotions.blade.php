...
@section('p2p_subtitle', __('Monitor active placements and promotion purchases.'))
    ...
    <div class="p2p-refresh">
        <section class="fb-card pa-table-card">
            ...
            <div class="fb-toolbar">
                <ul class="nav nav-pills p2p-admin-tabs" role="tablist" aria-label="{{ __('Promotion view') }}">
                    ...
                    <li class="nav-item" role="presentation">
                        <a class="nav-link {{ ! $isActive ? 'active' : '' }}"
                           ...
                           aria-current="{{ ! $isActive ? 'page' : 'false' }}">
                            <i class="fa-solid fa-receipt me-1" aria-hidden="true"></i>
            ...
            <form method="GET" action="{{ route('admin.p2p.promotions.index') }}" class="promo-filter">
                ...
                <div class="promo-filter__row">
                    @if($isActive)
                        <div class="promo-filter__field promo-filter__field--status">
                            <label for="promo_status">@lang('Status')</label>
                            <select name="status" id="promo_status" class="promo-filter__control">
                    ...
                    @endif
                    <div class="promo-filter__field promo-filter__field--offer">
                        <label for="promo_offer">@lang('Trade ad ID')</label>
                        <input type="number" name="offer_id" id="promo_offer" class="promo-filter__control" value="{{ request('offer_id') }}" min="1" placeholder="{{ __('e.g. 1234') }}">
                    </div>
                    <div class="promo-filter__field promo-filter__field--user">
                        <label for="promo_user">@lang('User')</label>
                        <input type="text" name="user_search" id="promo_user" class="promo-filter__control" value="{{ request('user_search') }}" placeholder="{{ __('Username or email') }}">
                    </div>
                    <div class="promo-filter__field promo-filter__field--plan">
                        <label for="promo_plan">@lang('Plan ID')</label>
                        <input type="number" name="package_id" id="promo_plan" class="promo-filter__control" value="{{ request('package_id') }}" min="1" placeholder="{{ __('e.g. 7') }}">
                    </div>
                    <div class="promo-filter__field promo-filter__field--trx">
                        <label for="promo_trx">@lang('Transaction ID')</label>
                        <input type="text" name="trx_id" id="promo_trx" class="promo-filter__control" value="{{ request('trx_id') }}" placeholder="TRX-...">
                    </div>
                                ...
                                <li class="promo-row">
                                    ...
                                    <div class="promo-meta">
                                        ...
                                        <span class="promo-meta__desc">
                                            ...
                                            @if($offerCurrency) &middot; {{ $offerCurrency }} @endif
                                            &middot; {{ $promotion->user?->name ?? __('Deleted user') }}
                                            @if($promotion->user)
                                    ...
                                    @foreach($purchases as $purchase)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if($offerCurrency)
                                                    <div class="fb-muted-xs">{{ $offerCurrency }}</div>
                                                @endif
                                            </td>
                                            <td>
                                                <div>{{ $purchase->user?->name ?? __('Deleted user') }}</div>
                                                @if($purchase->user)
                                                    <div class="fb-muted-xs">
                                            ...
                                            <td>
                                                ...
                                                <div class="fb-muted-xs">#{{ $purchase->package_id }}</div>
                                            </td>
                                            <td class="fb-mono">{{ $durationText }}</td>
                                            <td class="text-end fb-num">
                                                {{ number_format((float) $purchase->paid_amount, $decimals) }}
                                                <span class="fb-muted-xs">{{ $purchase->paid_currency }}</span>
                                            </td>
                                            <td class="fb-mono fb-muted-xs">
