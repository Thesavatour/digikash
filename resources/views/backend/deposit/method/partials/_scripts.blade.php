<script>
    $(document).ready(function () {
        ...
        function actionEvent(idName,isEdit = false) {
            ...
            // Generate a unique, editable method code
            $modalForm.on('click', '[data-generate-method-code]', function () {
                const $input = $(this).closest('.method-code-panel').find('[data-method-code]');
                if (!$input.length) {
                    return;
                }

                $input.val(generateMethodCode($modalForm, $(this).data('context'))).trigger('input').focus();
            });
            ...
            $modalForm.on('click', '#add-new-field', function () {
                const index = nextFieldIndex($modalForm);
                const formTemplate = `@include('backend.deposit.method.partials._method_append_form_field', ['key' => '${index}'])`;
        ...
        function generateMethodCode($modalForm, context) {
            const slugify = (text) => (text || '').toString().toLowerCase().trim()
                .replace(/[^a-z0-9]+/g, '-')
                .replace(/^-+|-+$/g, '');

            const name = $modalForm.find('[name="name"]').val();
            const currency = $modalForm.find('[name="currency"]').val();

            let base = [slugify(name), slugify(currency)].filter(Boolean).join('-');
            if (!base) {
                base = slugify(context) || 'method';
            }
        ...
        function nextFieldIndex($modalForm) {
            const indexes = $modalForm.find('[name^="fields["]').map(function () {
                const match = this.name.match(/^fields\[(\d+)]/);
