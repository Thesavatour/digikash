...
@push('styles')
    <link rel="stylesheet" href="{{ asset('backend/css/user-rank-admin.css?v=' . config('app.version')) }}">
@endpush
...
@section('content')
    ...
    <div class="card border-0 mb-4">
        <div class="card-body px-2">
                    ...
                    <table class="table user-table align-items-center">
                        ...
                        @foreach($userRanks as $userRank)
                            <tr class="align-middle">
                                <td>
                                    <div class="d-flex align-items-center">
                                        <img class="avatar rounded me-2" width="36" height="36"
                                             src="{{ asset($userRank->icon ?: 'general/static/default/rank.png') }}"
                                             alt="{{ $userRank->name }}" loading="lazy">
                                        <div>
                                ...
                                <td>
                                    @forelse((array) $userRank->transaction_types as $transactionType)
                                        <span class="badge bg-{{ TrxType::getBadgesColor([$transactionType]) }} form-badge-inline">
                                            {{ title($transactionType) }}
                                        </span>
                                    @empty
                                        <span class="text-muted small">{{ __('No transaction types') }}</span>
                                    @endforelse
                                </td>
                                <td>
                                    <span class="badge bg-{{ $userRank->is_active ? 'success' : 'danger' }}">
                                        {{ strtoupper($userRank->is_active ? __('active') : __('inactive')) }}
                                    </span>
                                </td>
                                <td class="fw-bold">
                                    <button type="button"
                                            data-edit-url="{{ route('admin.ranking.edit', $userRank->id) }}"
            ...
            @else
                <x-admin-not-found
                    :title="__('No user ranks configured')"
                    :message="__('Create the first rank to define wallet limits, referral levels, and progression rewards.')"
                    icon="fa-award"
                />
            @endif
        </div>
