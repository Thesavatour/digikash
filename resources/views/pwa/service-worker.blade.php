...
self.addEventListener("push", function (event) {
    event.waitUntil((async function () {
        ...
        var options = {
            ...
            data: payload.data || { url: "/user/notifications" },
            tag: payload.tag || "digikash-notification",
            renotify: payload.renotify !== false,
            requireInteraction: Boolean(payload.requireInteraction),
        };

        await self.registration.showNotification(payload.title || "DigiKash", options);
    })());
});
...
self.addEventListener("notificationclick", function (event) {
    ...
    var targetUrl = "/user/notifications";
    ...
    event.waitUntil((async function () {
        ...
        for (var i = 0; i < clientList.length; i++) {
            ...
            if (client.url.indexOf(self.location.origin) === 0 && "focus" in client) {
                ...
                // Fallback when navigate() is unavailable: open a fresh window.
                break;
            }
        }
