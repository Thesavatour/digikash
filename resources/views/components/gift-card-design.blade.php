...
@php
    ...
    $cardBackground = $imageUrl ? '#0F172A' : $t['bg'];
@endphp
<div class="gc-card"
     style="width:{{ $w }}px; height:{{ $h }}px; position:relative; background:{{ $cardBackground }}; color:{{ $t['ink'] }}; border-radius:{{ $radius }}px; overflow:hidden; box-shadow:{{ $shadow }}; font-family:'Plus Jakarta Sans', system-ui, sans-serif; flex:none;">
    ...
    {{-- Motif SVG overlays (skipped when a custom photo background is set) --}}
    @if(! $imageUrl)
    @switch($t['motif'])
        @case('confetti')
    ...
    {{-- Brand mark --}}
    @php $giftCardBrand = (string) setting('site_title', 'DigiKash'); @endphp
