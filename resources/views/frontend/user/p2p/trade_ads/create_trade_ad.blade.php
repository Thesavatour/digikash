...
@section('content')
    @php
        ...
        $manageFiatCurrency = $isEditing ? $offer->quote_currency_code : (siteCurrency('code') ?: $manageCurrency);
        $managePriceText = $isEditing ? number_format((float) $offer->price, $manageDecimals) . ' ' . $manageFiatCurrency : null;

        $fiatCurrencyOptions = collect($fiatCurrencies ?? []);
        $selectedWalletId = (int) old('wallet_id', $isEditing ? (int) $offer->wallet_id : 0);
        $selectedWallet = collect($wallets)->firstWhere('id', $selectedWalletId) ?? collect($wallets)->first();
        $assetCurrencyCode = $selectedWallet?->currency?->code ?? '';
        $selectedQuoteCurrencyId = (int) old('quote_currency_id', $isEditing ? (int) $offer->quote_currency_id : (int) ($defaultQuoteCurrencyId ?? 0));
        $selectedQuoteCurrency = $fiatCurrencyOptions->firstWhere('id', $selectedQuoteCurrencyId) ?? $fiatCurrencyOptions->first();
        $selectedQuoteCurrencyCode = $selectedQuoteCurrency?->code ?? (siteCurrency('code') ?: $assetCurrencyCode);
            ...
            <form method="POST" action="{{ $isEditing ? route('user.p2p.offers.update', $offer) : route('user.p2p.offers.store') }}">
                ...
                <div class="p2p-offer-section">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12 col-md-6 col-xl-4">
                            <label class="form-label">@lang('Price Currency (Fiat)')</label>
                            <select name="quote_currency_id" id="p2pQuoteCurrency" class="form-select form-select-sm" required>
                                @forelse($fiatCurrencyOptions as $fc)
                                    <option value="{{ $fc->id }}" data-code="{{ $fc->code }}" @selected((int) $selectedQuoteCurrencyId === (int) $fc->id)>{{ $fc->code }} — {{ $fc->name }}</option>
                                @empty
                                    <option value="">@lang('No fiat currency available')</option>
                                @endforelse
                            </select>
                            @error('quote_currency_id')
                        ...
                        <div class="col-12 col-md-6 col-xl-4">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <span class="input-group-text" id="p2pAmountCurrencyPrice">{{ $selectedQuoteCurrencyCode }}</span>
                            </div>
                            <small class="text-muted p2p-offer-muted">@lang('Buyers pay this price in the selected fiat for each unit of the asset.')</small>
                        </div>
                        ...
                        <div class="col-12 col-md-6 col-xl-4">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <span class="input-group-text" id="p2pAmountCurrencyMin">{{ $assetCurrencyCode }}</span>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-xl-4">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <span class="input-group-text" id="p2pAmountCurrencyMax">{{ $assetCurrencyCode }}</span>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-xl-4">
