...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dom = {
        ...
        quoteCurrencySelect: document.getElementById('p2pQuoteCurrency'),
    ...
    const syncAmountCurrency = function () {
        ...
        const assetCurrency = getDataAttr(walletOption, 'data-currency', FALLBACK_CURRENCY);

        [dom.amountCurrencyMin, dom.amountCurrencyMax].forEach(function (el) {
            setText(el, assetCurrency);
        });
    };

    const syncPriceCurrency = function () {
        const quoteOption = getSelectedOption(dom.quoteCurrencySelect);
        const fiatCurrency = getDataAttr(quoteOption, 'data-code', FALLBACK_CURRENCY);

        setText(dom.amountCurrencyPrice, fiatCurrency);
    };
    ...
    // Dashboard selects are enhanced into .dk-premium-select, which fires the
    // change via jQuery ($select.trigger('change')) — a native addEventListener
    // never receives it. Bind selects through jQuery so the dependent UI (price /
    // asset currency, package preview, method filters) stays in sync.
    const bindSelectChange = function (element, handler) {
        if (!element) {
            return;
        }
    ...
    const bindEvents = function () {
        bindSelectChange(dom.offerWalletSelect, syncAmountCurrency);
        bindSelectChange(dom.quoteCurrencySelect, syncPriceCurrency);
        bindSelectChange(dom.paymentMethodCountry, applyMethodFilters);
        bindSelectChange(dom.packageSelect, updatePromotionPreview);
        bindSelectChange(dom.sideSelect, filterPackagesBySide);
        bindSelectChange(dom.sideSelect, syncPaymentMethodSectionBySide);
