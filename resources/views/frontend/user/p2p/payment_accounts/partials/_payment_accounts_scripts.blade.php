...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // The method <select> is enhanced into a .dk-premium-select that fires the
    // change via jQuery ($select.trigger('change')), which native addEventListener
    // handlers never receive. Bind through jQuery so the dynamic fields reload on
    // every selection; fall back to a native listener when jQuery is unavailable.
    const bindChange = function (element, handler) {
        if (!element) {
            return;
        }
    ...
    const renderMethodInfo = function (target, method) {
        ...
        target.innerHTML = '<span class="p2p-account-info__icon" aria-hidden="true"><i class="fas fa-circle-info"></i></span>'
            + '<span class="p2p-account-info__body">' + body + '</span>';
    ...
    bindChange(createMethodSelect, function () {
    ...
    bindChange(editMethodSelect, function () {
