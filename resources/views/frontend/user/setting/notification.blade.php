                    ...
                    <div class="notification-toggle-stack">
                        ...
                        <div class="notification-toggle-card">
                            ...
                            <label class="notification-toggle-card__body" for="notifications_enabled">
                                <span class="notification-toggle-card__title">{{ __('Enable notifications') }}</span>
                                <span class="notification-toggle-card__text">{{ __('Show alerts in DigiKash and allow lock-screen / banner notifications on this device.') }}</span>
                                <span class="notification-toggle-card__hint small text-muted d-block mt-1" data-dk-webpush-hint></span>
                            </label>
                        </div>
                        ...
                        <div class="notification-toggle-card">
                            ...
                            <label class="notification-toggle-card__body" for="tune_enabled">
                                ...
                                <span class="notification-toggle-card__text">{{ __('Play the selected sound when alerts arrive while DigiKash is open.') }}</span>
                            </label>
                        </div>
                    </div>
            ...
            @if($notifications->isNotEmpty())
                <ul class="notification-all-list notification-feed-list list-group list-group-flush">
                    @foreach($notifications as $notification)
                        @php
                            // Defensive normalisation: legacy / framework / template-less
                            // notifications can persist a `data` payload that is empty or
                            // missing keys (e.g. a notification whose template has no push
                            // channel is stored as `data = []`). Accessing `$data['icon']`
                            // directly then throws "Undefined array key" and 500s the whole
                            // feed. Coalesce every key to a safe default so any payload
                            // shape renders cleanly.
                            $data   = is_array($notification->data) ? $notification->data : [];
                            $isRead = ! is_null($notification->read_at);

                            $notificationIcon   = $data['icon'] ?? 'bell';
                            $notificationAction  = (string) ($data['action_type'] ?? '');
                            $notificationClass   = NotificationActionType::getClass($notificationAction);
                            $notificationTitle   = $data['title'] ?? __('No message available');
                            $notificationMessage = $data['message'] ?? '';
                        @endphp

                        <li class="notification-item notification-feed-item list-group-item @unless($isRead) notification-item--unread @endunless">
                            <a
                                href="{{ route('user.notifications.markAsRead', $notification->id) }}"
                                class="notification-item__link text-decoration-none text-reset d-flex align-items-start gap-3 w-100"
                            >
                                <div class="notification-item__icon">
                                    <x-icon
                                        name="{{ $notificationIcon }}"
                                        class="notification-item__icon-svg text-{{ $notificationClass }}"
                                        height="38"
                                        width="38"
                                    />
                                </div>

                                <div class="notification-item__content flex-grow-1">
                                    ...
                                    <time class="notification-item__time d-block mt-1">
