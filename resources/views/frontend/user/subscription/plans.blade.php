        ...
        <div class="card single-form-card">
            ...
            <div class="card-body">
                ...
                @else
                                    ...
                                    <div class="sp-card__top">
                                        <div class="sp-card__header">
                                            ...
                                            <div class="sp-card__identity">
                                                ...
                                                @php
                                                    $planDescription = $plan->description ?: __('Access to core platform features.');
                                                @endphp
                                                <div class="sp-card__tagline"
                                                     ...
                                                     data-bs-custom-class="sp-plan-description-tooltip"
                                                     data-bs-title="{{ $planDescription }}">
                                                    {{ $planDescription }}
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <ul class="sp-card__feature-list">
                                                @foreach($plan->features as $feature)
                                                    @php
                                                        ...
                                                        $valueLabel = $feature->displayValue();
                                                        $pillClass = $isLockedFeature
                                                            ? 'sp-pill--locked'
                                                            : ($feature->isAccess() ? 'sp-pill--access' : ($feature->isBenefit() ? 'sp-pill--benefit' : 'sp-pill--value'));
                                                    @endphp
                                                    <li class="sp-card__feature-item {{ $isLockedFeature ? 'is-muted' : '' }}">
                                                        <span class="sp-card__feat-check {{ $isLockedFeature ? 'is-locked' : '' }}">
                                                            <i class="fas {{ $isLockedFeature ? 'fa-lock' : 'fa-check' }}"></i>
                                                        </span>
                                                        <span class="sp-card__feat-label">{{ $feature->feature_label }}</span>
                                                        <span class="sp-pill {{ $pillClass }} {{ $isFeatured && ! $isLockedFeature ? 'sp-pill--value-inv' : '' }}">
                                                            {{ $valueLabel }}
                                                        </span>
                                                    </li>
                                                @endforeach
                    ...
                    @php
                        $comparisonRows = [
                            ['label' => __('P2P marketplace'), 'key' => 'p2p_marketplace_browse', 'icon' => 'fa-store'],
                            ['label' => __('Buy / Sell P2P'), 'key' => 'p2p_trading', 'icon' => 'fa-handshake'],
                            ['label' => __('P2P ads'), 'key' => 'p2p_ad_limit', 'icon' => 'fa-bullhorn'],
                            ['label' => __('Virtual cards'), 'key' => 'virtual_card_limit', 'icon' => 'fa-credit-card'],
                            ['label' => __('Wallet Earn / staking'), 'key' => 'wallet_earn', 'icon' => 'fa-piggy-bank'],
                            ['label' => __('Withdrawal speed'), 'key' => 'withdrawal_speed', 'icon' => 'fa-bolt'],
                            ['label' => __('Support'), 'key' => 'support_priority', 'icon' => 'fa-headset'],
                        ];

                        $featureStateForPlan = function ($plan, string $key) {
                            $feature = $plan->features->firstWhere('feature_key', $key);

                            if (! $feature) {
                                return [
                                    'value'    => __('Not included'),
                                    'included' => false,
                                ];
                            }
                            ...
                            return [
                                'value'    => $value,
                                'included' => ! in_array(strtolower((string) $value), ['disabled', 'not included', '0'], true),
                            ];
                        };

                        $planMeta = function ($plan) {
                            $key = strtolower((string) ($plan->slug ?: $plan->name));

                            if (str_contains($key, 'starter') || $plan->isFree()) {
                                return ['tone' => 'starter', 'icon' => 'fa-paper-plane', 'subtitle' => __('Get started')];
                            }

                            if (str_contains($key, 'pro')) {
                                return ['tone' => 'pro', 'icon' => 'fa-star', 'subtitle' => __('Level up')];
                            }

                            if (str_contains($key, 'business') || str_contains($key, 'enterprise')) {
                                return ['tone' => 'business', 'icon' => 'fa-gem', 'subtitle' => __('Premium scale')];
                            }

                            return ['tone' => 'default', 'icon' => 'fa-layer-group', 'subtitle' => __('Plan access')];
                        };

                        $premiumPlan = $plans->first(fn($plan) => $plan->is_featured && ! $plan->isFree())
                            ?? $plans->first(fn($plan) => ! $plan->isFree());
                    @endphp

                    <section class="sp-compare" aria-labelledby="subscriptionComparisonTitle">
                        <div class="sp-compare__head">
                            <div class="sp-compare__intro">
                                <span class="sp-compare__eyebrow">{{ __('Plan comparison') }}</span>
                                <h3 id="subscriptionComparisonTitle">{{ __('Upgrade when your workflow needs more room') }}</h3>
                                <p>{{ __('Choose the plan that fits your goals today. Upgrade anytime as you grow.') }}</p>
                            </div>
                            <div class="sp-compare__promo">
                                <span class="sp-compare__promo-icon">
                                    <i class="fas fa-crown"></i>
                                </span>
                                <span class="sp-compare__promo-copy">
                                    <strong>{{ __('Premium Access') }}</strong>
                                    <small>{{ __('Unlock more features, higher limits and priority support.') }}</small>
                                </span>
                                <a href="{{ $premiumPlan ? route('user.subscription.checkout', ['plan' => $premiumPlan->slug, 'cycle' => 'monthly']) : route('user.subscription.plans') }}"
                                   class="sp-compare__promo-btn {{ $premiumPlan ? 'js-checkout-link' : '' }}"
                                   @if($premiumPlan)
                                       data-base="{{ route('user.subscription.checkout', ['plan' => $premiumPlan->slug, 'cycle' => 'monthly']) }}"
                                       data-slug="{{ $premiumPlan->slug }}"
                                   @endif>
                                    {{ __('Go Premium') }}
                                    <i class="fas fa-arrow-right"></i>
                                </a>
                        ...
                        <div class="sp-compare__table-wrap">
                                    ...
                                    <tr>
                                        ...
                                        @foreach($plans as $plan)
                                            @php
                                                $meta = $planMeta($plan);
                                            @endphp
                                            <th class="sp-compare__plan-head sp-compare__plan-head--{{ $meta['tone'] }}">
                                                <span class="sp-compare__plan-title">
                                                    <span class="sp-compare__plan-icon">
                                ...
                                <tbody>
                                    @foreach($comparisonRows as $row)
                                        <tr>
                                            ...
                                            @foreach($plans as $plan)
                                                @php
                                                    $state = $featureStateForPlan($plan, $row['key']);
                                                @endphp
                                                <td>
                                                    <span class="sp-compare__value {{ $state['included'] ? 'is-included' : 'is-muted' }}">
                                                        @if($state['included'])
                                                            <i class="fas fa-check-circle"></i>
                                                        @else
                        ...
                        <div class="sp-compare__trust">
                            <span class="sp-compare__trust-main">
                                <i class="fas fa-star"></i>
                                {{ __('All plans come with bank-level security and 24/7 system monitoring.') }}
                            </span>
                            <span><i class="fas fa-shield-alt"></i> {{ __('Secure') }}</span>
                            <span><i class="fas fa-clock"></i> {{ __('Reliable') }}</span>
                            <span><i class="fas fa-lock"></i> {{ __('Trusted') }}</span>
                        </div>
                    </section>
                @endif
...
(function () {
    ...
    if (typeof bootstrap !== 'undefined' && bootstrap.Tooltip) {
        document.querySelectorAll('[data-bs-custom-class="sp-plan-description-tooltip"]').forEach(function (el) {
