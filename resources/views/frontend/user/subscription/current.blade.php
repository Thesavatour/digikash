            ...
            <div class="card-main">
                ...
                @else
                    @php
                        ...
                        $enabledCount = $features->filter(
                            fn ($f) => (! $f->isAccess() && ! $f->isBenefit()) || ! $f->isDisabled()
                        )->count();
                    ...
                    <div class="ms-page" data-mobile-scroll-stable>
                        ...
                        <div class="ms-grid-2">
                            ...
                            <div class="ms-card">
                                ...
                                @php
                                    // Build usage meters plus access/benefit entitlements.
                                    $usageFeatures = $features->filter(fn ($f) => isset($usage[$f->feature_key]))->values();
                                    $entitlementFeatures = $features
                                        ->filter(fn ($f) => $f->isAccess() || $f->isBenefit())
                                        ->values();

                                    $iconMap = [
                                        'daily_transaction_limit'   => 'fa-calendar-day',
                                        ...
                                        'p2p_marketplace_browse'    => 'fa-store',
                                        'p2p_trading'               => 'fa-handshake',
                                        'virtual_card_limit'        => 'fa-credit-card',
                                        'virtual_cards'             => 'fa-credit-card',
                                        'wallet_earn'               => 'fa-seedling',
                                        'withdrawal_speed'          => 'fa-stopwatch',
                                        'p2p_ad_limit'              => 'fa-bullhorn',
                                        'p2p_trade_limit'           => 'fa-handshake',
                                        'support_priority'          => 'fa-headset',
                                        'api_access'                => 'fa-code',
                                    ];
                                @endphp

                                @if($usageFeatures->isEmpty() && $entitlementFeatures->isEmpty())
                                    <x-user-not-found
                                        class="mt-2"
                                    ...
                                    />
                                @else
                                    @if($entitlementFeatures->isNotEmpty())
                                        <div class="ms-entitlements">
                                            @foreach($entitlementFeatures as $feature)
                                                @php
                                                    $isMuted = $feature->isDisabled();
                                                    $iconCls = $iconMap[strtolower((string) $feature->feature_key)] ?? ($feature->isBenefit() ? 'fa-star' : 'fa-bolt');
                                                @endphp
                                                <div class="ms-entitlement {{ $isMuted ? 'is-muted' : '' }}">
                                                    <span class="ms-entitlement__icon">
                                                        <i class="fa-solid {{ $isMuted ? 'fa-lock' : $iconCls }}"></i>
                                                    </span>
                                                    <span class="ms-entitlement__body">
                                                        <span class="ms-entitlement__label">{{ $feature->feature_label }}</span>
                                                        <strong>{{ $feature->displayValue() }}</strong>
                                                    </span>
                                                    <span class="ms-entitlement__badge">
                                                        {{ $isMuted ? __('Locked') : ($feature->isBenefit() ? __('Benefit') : __('Access')) }}
                                                    </span>
                                                </div>
                                            @endforeach
                                        </div>
