            ...
            @forelse($giftCards as $card)
                ...
                <div class="gc-row">
                    <div class="gc-row-thumb">
                        <x-gift-card-design :preset="$preset" :amount="$card->amount" :recipient="$card->recipient_name" :sender="$card->sender_name" :width="102" :image="$card->template?->image" :background-color="$card->template?->background_color" :text-color="$card->template?->text_color" :ribbon-text="$card->template?->ribbon_text"/>
                    </div>
