...
<script>
    (function () {
        var openTrx = @json($openTrx ?? null);
        if (!openTrx) {
            return;
        }

        function openReceiptModal() {
            var modalEl = document.querySelector('.ud-trx-modal[data-trx-id="' + openTrx + '"]');
            if (!modalEl && openTrx) {
                // Case-insensitive fallback
                document.querySelectorAll('.ud-trx-modal[data-trx-id]').forEach(function (el) {
                    if ((el.getAttribute('data-trx-id') || '').toLowerCase() === String(openTrx).toLowerCase()) {
                        modalEl = el;
                    }
                });
            }
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', openReceiptModal);
        } else {
            openReceiptModal();
        }
    })();
