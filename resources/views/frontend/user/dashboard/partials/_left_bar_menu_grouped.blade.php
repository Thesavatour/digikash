<div class="single-card-box d-lg-block d-none dashboard-sidebar-menu-card">
	@php
		...
		$remittanceMenuVisible = $featureManager->isVisible('remittance');
	...
	<ul class="left-menu-box">
		...
		{{-- Group: Remittance (New Transfer, Recipients, History) --}}
		@if($remittanceMenuVisible)
			@php
				$remittanceActive = request()->routeIs('user.remittance.*')
					|| request()->routeIs('user.beneficiaries.*');
			@endphp
			<li class="menu-group">
				<a href="javascript:void(0)" class="group-toggle {{ $remittanceActive ? 'active' : '' }}" data-bs-toggle="collapse" data-bs-target="#menuRemittance"
				   aria-expanded="{{ $remittanceActive ? 'true' : 'false' }}" aria-controls="menuRemittance">
					<x-icon name="currency-exchange" class="icon"/>
					{{ __('Remittance') }}
					<i class="fa fa-angle-right arrow"></i>
				</a>
				<ul id="menuRemittance" class="collapse submenu {{ $remittanceActive ? 'show' : '' }}" data-bs-parent=".left-menu-box">
					<li>
						<a href="{{ route('user.remittance.create') }}" class="{{ isActive('user.remittance.create') }}">
							<x-icon name="send-money" class="icon"/>
							{{ __('New Transfer') }}
							<i class="fa fa-angle-right arrow"></i>
						</a>
					...
					<li>
						<a href="{{ route('user.beneficiaries.index') }}" class="{{ request()->routeIs('user.beneficiaries.*') ? 'active' : '' }}">
							<x-icon name="sidebar-users" class="icon"/>
							{{ __('Recipients') }}
							<i class="fa fa-angle-right arrow"></i>
						</a>
					</li>
					<li>
						<a href="{{ route('user.remittance.index') }}" class="{{ isActive('user.remittance.index') }}">
							<x-icon name="history" class="icon"/>
							{{ __('Transfer History') }}
							<i class="fa fa-angle-right arrow"></i>
						</a>
