@php
    ...
    $name           = (string) ($signupBonusPopup['name'] ?? '');
    ...
    $brand = trim((string) setting('signup_bonus_popup_brand', ''));
    if ($brand === '') {
        $brand = (string) setting('site_title', 'DigiKash');
    }

    $titleTemplate = (string) setting('signup_bonus_popup_title', 'Welcome to :brand, :name!');
    $subtitleTemplate = (string) setting(
        'signup_bonus_popup_subtitle',
        'Your welcome bonus has been added to your wallet. Spend it anywhere on :brand.'
    );
    $primaryButton = (string) setting('signup_bonus_popup_primary_button', 'View My Wallet');
    $secondaryButton = (string) setting('signup_bonus_popup_secondary_button', 'See Transaction');

    $brandToken = '___BRAND__';
    $nameToken = '___NAME__';
    $titleWithTokens = str_replace(
        [':brand', ':name'],
        [$brandToken, $nameToken],
        $titleTemplate
    );
    $titleHtml = e($titleWithTokens);
    $titleHtml = str_replace(
        [e($brandToken), e($nameToken)],
        ['<span class="signup-bonus-brand">'.e($brand).'</span>', e($name)],
        $titleHtml
    );
    $titleHtml = preg_replace('/,\s+/', ',<br/>', $titleHtml, 1) ?? $titleHtml;

    $subtitleText = str_replace(
        [':brand', ':name'],
        [$brand, $name],
        $subtitleTemplate
    );
            ...
            <div class="signup-bonus-actions">
                <a href="{{ $walletUrl }}"
                   ...
                   data-signup-bonus-close>
                    ...
                    <span class="signup-bonus-btn-label">{{ $primaryButton }}</span>
