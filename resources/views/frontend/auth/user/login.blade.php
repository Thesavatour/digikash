    ...
    <div class="auth-shell">
        <div class="auth-card has-accent is-user">
            ...
            <div class="auth-head">
                ...
                <h4 class="auth-title">{{ __('User Login') }}</h4>
                <p class="auth-subtitle">{{ __('Sign in to your user account at') }} {{ setting('site_title') }}</p>
            ...
            <form id="userLoginForm" action="{{ route('user.login') }}" method="post">
                ...
                <button class="btn btn-primary btn-submit-auth" type="submit">
            ...
            <p class="text-center mt-4 mb-2">
                {{ __("Don't have a user account?") }}
                <a href="{{ route('user.register') }}" class="text-decoration-none text-primary fw-semibold">{{ __('Create One') }}</a>
            </p>

            <x-demo-credentials portal="user" form-id="userLoginForm" />
