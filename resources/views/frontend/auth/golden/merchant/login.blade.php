	...
	<section class="shell">
		<div class="form-panel">
			...
			<div class="form-body">
				...
				<div class="eyebrow form-eyebrow">{{ __('Merchant Portal') }}</div>
				<h1 class="form-title">
					{!! __('Resume your :em counter.', ['em' => '<em class="italic-gold">'.__('private').'</em>']) !!}
				</h1>
				<p class="form-desc">{{ __('Accept payments, generate QR codes, and review settlements — with the composure of a private bank.') }}</p>

				<form action="{{ route('merchant.login') }}" method="POST" novalidate>
					@csrf
					...
					<div class="field">
						...
						<div class="field__wrap has-lead">
							...
							<input id="gdk-login" name="login" type="text" placeholder="{{ __('Your merchant email') }}" value="{{ old('login') }}" required autofocus>
						</div>
					</div>
					...
					<div class="row">
						...
						<a href="{{ route('merchant.password.request') }}" class="gold-link gold-link--caps">{{ __('Forgot password?') }}</a>
					</div>
					...
					@include('frontend.auth.partials._social_login', ['portal' => 'merchant'])

					<div class="or-div">{{ __('Or') }}</div>

					<div class="muted-note--centered">
						{{ __("Don't have a merchant account?") }}
						<a href="{{ route('merchant.register') }}" class="gold-link">{{ __('Open a Merchant Account') }}</a>
					</div>

					@include('frontend.layouts.golden.partials._auth_portal_switch', ['current' => 'merchant', 'page' => 'login'])
				</form>
			</div>
		</div>
