...
@section('title', __('Create Agent Account'))
	...
	<section class="shell form-register">
		<div class="form-panel">
			...
			<div class="form-body">
				...
				<div class="eyebrow form-eyebrow">{{ __('Apply as an Agent') }}</div>
				<h1 class="form-title">
					{!! __('Earn :em on every referral.', ['em' => '<em class="italic-gold">'.__('commission').'</em>']) !!}
				</h1>
				<p class="form-desc">{{ __('Submit your application — admin reviews each agent personally. Approval unlocks your dashboard and commission tracking.') }}</p>

				<form action="{{ route('agent.register') }}" method="POST" novalidate>
					@csrf
					...
					<button type="submit" class="btn btn--filled btn--full u-mt-8">
						{{ __('Submit Application') }} <i class="fa-solid fa-arrow-right"></i>
					</button>

					@include('frontend.auth.partials._social_login', ['portal' => 'agent'])

					<div class="form-footer">
						{{ __('Already have an agent account?') }}
						<a href="{{ route('agent.login') }}" class="gold-link">{{ __('Sign In') }}</a>
					</div>

					@include('frontend.layouts.golden.partials._auth_portal_switch', ['current' => 'agent', 'page' => 'register'])
				</form>
			</div>
		</div>
