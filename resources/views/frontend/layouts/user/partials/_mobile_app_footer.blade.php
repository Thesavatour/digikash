...
<div class="dk-sheet dk-more-sheet d-lg-none" role="dialog" aria-label="{{ __('More services') }}" aria-hidden="true">
    ...
    <div class="dk-more-menu">
        ...
        @php
            ...
            $moreItems[] = [
                'label' => __('Gift Cards'),
                'route' => 'user.gift-card.index',
                'icon' => 'fas fa-gift',
                'tone' => 'green',
                'sub' => __('Send & redeem gift cards'),
                'feature' => 'gift_cards',
            ];
            ...
            if (
                ...
                && \Illuminate\Support\Facades\Route::has('user.wallet-earn.stakes')
            ) {
            ...
            if (
                $mobileFooterUser
                && \Illuminate\Support\Facades\Route::has('user.subscription.current')
            ) {
                $activeSubscription = $mobileFooterUser->activeSubscription()
                    ->with('plan')
                    ->first();

                if ($activeSubscription || $featureManager->isVisible('subscription_system')) {
                    ...
                    $mobileMoreHighlights[] = [
                        ...
                        'route' => 'user.subscription.current',
        ...
        @php
            $mobileMoreHighlights = array_values(array_filter(
                $mobileMoreHighlights,
                fn (array $h): bool => isset($h['route']) && \Illuminate\Support\Facades\Route::has($h['route'])
            ));
        @endphp
            ...
            <section class="dk-more-overview" aria-label="{{ __('Current status') }}">
                ...
                <div class="dk-more-overview__grid">
                    @foreach($mobileMoreHighlights as $highlight)
                        <a href="{{ route($highlight['route'], $highlight['params'] ?? []) }}" class="dk-more-overview-card" data-tone="{{ $highlight['tone'] }}" data-state="{{ $highlight['state'] }}" role="link">
