...
use App\Http\Controllers\Frontend\Auth\Agent\AuthenticatedSessionController as AgentAuthenticatedSessionController;
use App\Http\Controllers\Frontend\Auth\Agent\NewPasswordController as AgentNewPasswordController;
use App\Http\Controllers\Frontend\Auth\Agent\PasswordResetLinkController as AgentPasswordResetLinkController;
use App\Http\Controllers\Frontend\Auth\Agent\RegisteredUserController as AgentRegisteredUserController;
...
use App\Http\Controllers\Frontend\Auth\User\SocialAuthController;
...
/*
|--------------------------------------------------------------------------
| Login / Register URL Aliases
|--------------------------------------------------------------------------
|
| The real frontend auth screens live under the /user/ prefix below.
| These short aliases catch visitors who type /login or /register by
| reflex or follow stale bookmarks. 301 because the mapping is stable —
| browsers and search engines can cache the redirect aggressively.
|
| The CMS catchall in routes/web.php excludes these slugs from its
| negative-lookahead regex so a CMS page can never shadow them.
*/
Route::redirect('/login', '/user/login', 301);
Route::redirect('/signin', '/user/login', 301);
Route::redirect('/register', '/user/register', 301);
Route::redirect('/signup', '/user/register', 301);
...
Route::middleware('guest')->group(function () {
    ...
    // Agent Authentication Routes (prefix: 'agent')
    Route::prefix('agent')->name('agent.')->group(function () {
        // Registration routes
        Route::get('register', [AgentRegisteredUserController::class, 'create'])->name('register');
        Route::post('register', [AgentRegisteredUserController::class, 'store']);

        // Authentication routes
        Route::get('login', [AgentAuthenticatedSessionController::class, 'create'])->name('login');
        Route::post('login', [AgentAuthenticatedSessionController::class, 'store']);
        ...
        Route::get('reset-password/{token}', [AgentNewPasswordController::class, 'create'])->name('password.reset');
        Route::post('reset-password', [AgentNewPasswordController::class, 'store'])->name('password.store');
    });
...
Route::group(['middleware' => ['XSS'], 'as' => 'admin.', 'prefix' => $adminPrefix], function () {
