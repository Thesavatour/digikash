...
(function () {
    ...
    var WEBPUSH_VAPID_URL = "/user/web-push/vapid-public-key";
    var WEBPUSH_SUBSCRIBE_URL = "/user/web-push/subscribe";
    var WEBPUSH_UNSUBSCRIBE_URL = "/user/web-push/unsubscribe";
    ...
    window.addEventListener("load", function () {
        ...
        navigator.serviceWorker.register("/service-worker.js", { scope: SERVICE_WORKER_SCOPE })
            .then(function (serviceWorkerRegistration) {
                ...
                maybeSyncWebPushSubscription();
                refreshWebPushButtonState();
            })
    ...
    window.DKPwa = {
        ...
        disableWebPush: disableWebPush,
        webPushSupported: webPushSupported,
        webPushPermission: function () {
    ...
    function fetchVapidPublicKey() {
        return fetch(WEBPUSH_VAPID_URL, {
            credentials: "same-origin",
            headers: csrfHeaders(),
        }).then(function (response) {
            if (!response.ok) {
                throw new Error("Unable to load Web Push configuration.");
            }
    ...
    function subscriptionToPayload(subscription) {
        ...
        return {
            ...
            encoding: (PushManager.supportedContentEncodings && PushManager.supportedContentEncodings[0]) || "aes128gcm",
        };
    }

    function persistSubscription(subscription) {
        return fetch(WEBPUSH_SUBSCRIBE_URL, {
            method: "POST",
            credentials: "same-origin",
            headers: csrfHeaders(),
    ...
    function removeSubscription(endpoint) {
        return fetch(WEBPUSH_UNSUBSCRIBE_URL, {
            method: "DELETE",
            credentials: "same-origin",
            headers: csrfHeaders(),
    ...
    function maybeSyncWebPushSubscription() {
        if (!webPushSupported()) {
            return;
        }
        ...
        // Permission alone is not enough — ensure we have a PushSubscription saved.
        enableWebPush().catch(function (error) {
            if (window.console && window.console.warn) {
                window.console.warn("[DigiKash Web Push] sync failed:", error);
            }
            refreshWebPushButtonState();
        });
    }
    ...
    function enableWebPush() {
        if (!webPushSupported()) {
            return Promise.reject(new Error("Device notifications are not supported in this browser."));
        }

        return fetchVapidPublicKey().then(function (config) {
            if (!config.enabled || !config.public_key) {
                throw new Error("Device notifications are not configured on the server.");
            }
            ...
            return permissionPromise.then(function (permission) {
                ...
                return getReadyRegistration().then(function (serviceWorkerRegistration) {
                    return serviceWorkerRegistration.pushManager.getSubscription().then(function (existing) {
                        if (existing) {
                            return existing;
                        }
                        ...
                        return persistSubscription(subscription).then(function () {
                            refreshWebPushButtonState();
                            window.dispatchEvent(new CustomEvent("dk:webpush-enabled"));
        ...
        return getReadyRegistration().then(function (serviceWorkerRegistration) {
            return serviceWorkerRegistration.pushManager.getSubscription().then(function (subscription) {
                ...
                return subscription.unsubscribe().then(function () {
                    return removeSubscription(endpoint).catch(function () {});
                }).then(function () {
                    refreshWebPushButtonState();
                    window.dispatchEvent(new CustomEvent("dk:webpush-disabled"));
    ...
    function setWebPushHint(message) {
        document.querySelectorAll("[data-dk-webpush-hint]").forEach(function (hint) {
            hint.textContent = message || "";
        });
    }

    function refreshWebPushButtonState() {
        var toggle = document.querySelector("[data-dk-webpush-toggle]");
        var legacyButton = document.querySelector("[data-dk-webpush-enable]");

        if (isInsecureContext) {
            setWebPushHint("HTTPS is required for lock-screen notifications on this device.");
            if (legacyButton) {
        ...
        if (permission === "granted") {
            setWebPushHint("Device alerts are allowed. Lock-screen notifications can reach this phone.");
            if (legacyButton) {
                legacyButton.setAttribute("data-dk-webpush-state", "enabled");
            }
        } else if (permission === "denied") {
            setWebPushHint("Notifications are blocked in browser/OS settings. Allow DigiKash, then turn this on again.");
            if (legacyButton) {
                legacyButton.setAttribute("data-dk-webpush-state", "denied");
            }
            if (toggle && toggle.checked) {
                // Keep preference, but surface that OS permission is blocked.
            }
        } else {
            setWebPushHint("Turning this on will ask for permission to send lock-screen alerts.");
            if (legacyButton) {
                legacyButton.setAttribute("data-dk-webpush-state", "default");
            }
        }
    ...
    document.addEventListener("change", function (event) {
        var toggle = event.target && event.target.closest
            ? event.target.closest("[data-dk-webpush-toggle]")
            : null;
        if (!toggle) {
            return;
        }
        ...
        if (toggle.checked) {
            enableWebPush().catch(function (error) {
                refreshWebPushButtonState();
                if (window.console && window.console.warn) {
                    window.console.warn("[DigiKash Web Push]", error);
                }
                window.alert(error && error.message ? error.message : "Unable to enable device notifications.");
            });

            return;
        }

        disableWebPush().catch(function (error) {
            if (window.console && window.console.warn) {
                window.console.warn("[DigiKash Web Push]", error);
            }
        });
    ...
    document.addEventListener("click", function (event) {
        var target = event.target instanceof Element ? event.target : event.target.parentElement;
        var button = target ? target.closest("[data-dk-webpush-enable]") : null;
        if (!button) {
            return;
        }

        event.preventDefault();
        var state = button.getAttribute("data-dk-webpush-state") || "default";
    ...
    window.addEventListener("load", function () {
        ...
        var toggle = document.querySelector("[data-dk-webpush-toggle]");
        if (toggle && toggle.checked && webPushSupported() && Notification.permission !== "denied") {
            maybeSyncWebPushSubscription();
        }
    });
})();
