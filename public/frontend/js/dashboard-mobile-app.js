...
(function () {
    ...
    function initScrollBehavior() {
        ...
        if (document.querySelector("[data-mobile-scroll-stable]")) {
            if (header) header.setAttribute("data-scrolled", "0");
            if (tabbar) tabbar.setAttribute("data-hidden", "0");
        ...
        var lastTabbarState = null;

        function update() {
            var y = window.scrollY || window.pageYOffset || 0;
            var nextHeaderState = y > 8 ? "1" : "0";
            var nextTabbarState = "0";

            if (header && lastHeaderState !== nextHeaderState) {
                header.setAttribute("data-scrolled", nextHeaderState);
                lastHeaderState = nextHeaderState;
            }

            if (tabbar) {
                if (lastTabbarState !== nextTabbarState) {
                    tabbar.setAttribute("data-hidden", nextTabbarState);
                    lastTabbarState = nextTabbarState;
                }
            }
