...
(function ($) {
    ...
    const initGatewayCredentialHealth = function () {
        $(document).on("click", "[data-gateway-test]", function () {
            const $btn = $(this);
            const $form = $btn.closest("form");
            const $result = $form.find("[data-gateway-test-result]");
            const $label = $btn.find("[data-gateway-test-label]");
            const $icon = $btn.find(".pgm-test-btn__icon i");
            const defaultLabel = $label.text();
            const defaultIconClass = $icon.attr("class");
            const testUrl = $btn.data("test-url");
            const testingLabel = $btn.data("testing-label") || i18n.testing;
            const testingMessage = $btn.data("testing-message") || i18n.probing;
            const fallbackMessage = $btn.data("fallback-message") || i18n.testFailed;

            if (!testUrl || !$result.length || $btn.prop("disabled")) {
                return;
            }
